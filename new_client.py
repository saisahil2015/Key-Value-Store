...
def kv_store_client(operation, key, value=None):
    ...
    try:
        if operation == 'set':
            requests.put(f"{server_url}/store", params={"key": key}, data={"value": value})
        elif operation == 'get':
...
def benchmark(num_operations, num_processes):
    ...
    total_operations = num_operations * num_processes * 3  # Each process does num_operations SET and GET
    total_latencies = []
